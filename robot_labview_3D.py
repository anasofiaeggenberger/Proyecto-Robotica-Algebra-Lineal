...
def animate_robot_3D():
    ...
    # --- PLANO BASE (Futurista Azul) ---
    grid_size = np.linspace(-6, 6, 30)
    X, Y = np.meshgrid(grid_size, grid_size)
    Z = np.zeros_like(X)
    ax.plot_surface(X, Y, Z, color='#0F1116', alpha=0.5, edgecolor='#333333', linewidth=0.3)

    # --- SOMBRA ---
    shadow_line, = ax.plot([], [], [], color='#333333', lw=2, alpha=0.4)

    # --- PANEL DERECHO DE MATRICES ---
    ...
    trail, = ax.plot([], [], [], color='white', lw=1.5, alpha=0.6)
    effector_point = None  # se definirá dinámicamente
    ...
    def update(frame):
        ...
        # Sombra proyectada en el plano (Z=0)
        shadow_line.set_data(xs, ys)
        shadow_line.set_3d_properties(np.zeros(len(xs)))

        # --- EFECTOR FINAL (punto rojo brillante) ---
        if effector_point:
        ...
        # Mostrar también en la terminal
        print("\n========================== FRAME", frame, "==========================")
        print(matrices_text)

        # Movimiento de cámara (rotación sutil)
        ax.view_init(elev=25 + np.sin(frame/40)*5, azim=35 + frame/2)

        return line, trail, shadow_line, text_display, effector_point
